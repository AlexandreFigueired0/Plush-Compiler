...
from src.type_checker.type_checker import type_check_program
import sys
...
def plush_type_to_llvm_type(type_):
    ...
    match type_:
        case IntType(_,_,_,_,):
            return ir.IntType(32)
        case FloatType(_,_,_,_,):
            return ir.FloatType()
        case CharType():
            return ir.IntType(8)
        case StringType(_,_,_,_,):
            return ir.PointerType(ir.IntType(8))
        case BooleanType(_,_,_,_,):
            return ir.IntType(1)
        case ArrayType(_,_,_,_,_):
...
def compile(emitter: Emitter, node):
    ...
    match node:
        case Start( defs_or_decls ):
            first_traversal(emitter, node)
            predef_funcs_file = open("resources/pre_def_funcs.ll", "r")
